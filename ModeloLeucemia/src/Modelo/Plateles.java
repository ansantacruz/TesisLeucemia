...
public class Plateles implements Cell {
	...
	public Plateles(boolean healthParameter, int timeOfLife, int MinimalPerception, int IterationsToEendSignal) {
	...
	public String ReceiveNeighbors(int[] quantityNeighbors, int i) {
		...
		detectedNeighbors = detectedNeighbors + quantityNeighbors[0] + quantityNeighbors[1];
		...
		if (iterationSignalCounter <= iterationSignalCounter) {
			if (iterationsCounters > timeOfLifeCell) {
				...
				if (health == true) {
					if (quantityNeighbors[0] == 8 || quantityNeighbors[0] <= 1 || quantityNeighbors[5] >= 3
							|| quantityNeighbors[4] >= 2) {
						Action = "Die";
					}
				} else {
	...
	public String Apoptosis() {
		return "DieAndSeendSignalPlatetesBloodCells";
	}
